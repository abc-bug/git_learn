...
public class TransportAnalyse extends DataFilter implements IDataAnalyse {
	// 发货集合
	private ArrayList<Transport> transSends = new ArrayList<Transport>(); //same ,add
	// 送货集合
	private ArrayList<Transport> transIngs = new ArrayList<Transport>();  //same,add
	// 已签收集合
	private ArrayList<Transport> transRecs = new ArrayList<Transport>();  //same.add
	...
	public ArrayList<MatchedTransport> matchData() {
		// 创建物流匹配集合
		ArrayList<MatchedTransport> matchTrans = new ArrayList<MatchedTransport>(); //same,add
		// 数据匹配分析
			...
			for (Transport tran : transIngs) {
				for (Transport rec : transRecs) {
					if ((send.getReciver().equals(tran.getReciver()))
						...
						// 修改物流状态类型为“匹配”
						if((send.getType()!=DataBase.MATHCH)&&(tran.getType()!=DataBase.MATHCH)&&(rec.getType()!=DataBase.MATHCH)){
